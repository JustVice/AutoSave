...
public class Data {
    ...
    public void LoadData() {
        globalDir();
        dirFiles(Memory.folderForDataName);
    ...
    private void dirFiles(String folderName) {
        String path = System.getenv("APPDATA") + "\\"+Memory.globalFolderForData+"\\" + folderName;
    ...
    private void globalDir() {
        String path = System.getenv("APPDATA") + "\\" + Memory.globalFolderForData;
        File file = new File(path);
        if (!file.exists()) {
            boolean created = file.mkdir();
            if (created) {
